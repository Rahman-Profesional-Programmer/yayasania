...
if ($conn->connect_error) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($conn, $sql)) {
      ...
      echo '<script>Swal.fire({icon:"success",title:"Berhasil",text:"Menu berhasil diedit",confirmButtonColor:"#0d6efd"}).then(function(){window.location.href="crud-menu.php";});</script>';
    } else {
      echo '<link rel="stylesheet" href="assets/plugins/sweetalert2/css/sweetalert2.min.css">';
      echo '<script src="assets/plugins/sweetalert2/js/sweetalert2.min.js"></script>';
      echo '<script>Swal.fire({icon:"error",title:"Gagal",text:"Menu gagal diedit",confirmButtonColor:"#0d6efd"}).then(function(){window.location.href="crud-menu.php";});</script>';
    }
