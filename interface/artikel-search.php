                        ...
                        <ul class="header--topbar-info nav">
                            <li><i class="fa fm fa-calendar"></i>
                                <?php 
                                    ...
                                    $hari_ini = $nama_hari[$hari-1]; // mengambil nama hari dari array
                                    $tanggal = date("j");
                                    $bulan = date("F");
                                    $tahun = date("Y");
        ...
        <div class="main--breadcrumb">
            <div class="container">
                <ul class="breadcrumb">
                    ...
                    <li><a href="artikel-list.php" class="btn-link"><span>List Artikel</span></a></li>
                            ...
                            <div class="post--items post--items-5 pd--30-0">
                                ...
                                <?php
                                    ...
                                    if (isset($_GET['search'])) {
                                        ...
                                        WHERE disable = 1 AND hapus = 1 
                                        AND (judul_artikel LIKE '%$search%' OR konten_artikel LIKE '%$search%')";

                                    } else if (isset($_GET['cari_ketegori'])) {

                                        $search = $_GET['cari_ketegori'];

                                        //query untuk mengambil data dari tabel Artikel
                                        $query_artikel = "SELECT * FROM artikel 
                                        WHERE disable = 1 AND hapus = 1 
                                        AND (kategori LIKE '%$search%' OR konten_artikel LIKE '%$search%')";
                                        
                                    } else if (isset($_GET['cari_tag'])){

                                        $search_tag = $_GET['cari_tag'];
                                        // echo "Tag ada isinya. <br>";
                                        // echo "<br>";
                                        // echo var_dump($search_tag); echo "<------------ isi string. <br>";

                                        // query untuk mengambil data dari tag
                                        // $query_tag = "SELECT*FROM artikel_tag WHERE tag LIKE '%$search_tag%'";
                                        $query_tag =  "SELECT *FROM artikel
                                        JOIN artikel_tag ON artikel.id_artikel = artikel_tag.id_artikel
                                        WHERE artikel_tag.tag LIKE '%$search_tag%'";
                                        ...
                                        while ($row_tag = mysqli_fetch_assoc($result_tag)) {
                                            ...
                                            // echo "<br>";
                                            // echo $row_tag['id_artikel']." <--- ID artikel";
                                            // echo "<br>";
                                            $id_artikel = $row_tag['id_artikel'];

                                            // Proses array_push
                                            array_push($array_id, $id_artikel);
                                        }

                                    } else if (isset($_GET['cari_by_date'])){

                                        // echo $_GET['cari_by_date']; 
                                        // echo "<br>";

                                        $nilai = $_GET['cari_by_date']; // Mendapatkan nilai dari parameter 'param'
                                        $nilaiArray = explode(',', $nilai); // Memisahkan nilai menjadi array

                                        // Mengakses nilai-nilai yang terpisah
                                        // foreach ($nilaiArray as $value) {
                                        // echo $nilaiArray[0] . "<br>";
                                        // echo $nilaiArray[1] . "<br>";
                                        // }

                                        $query_artikel =  "SELECT*FROM artikel WHERE MONTH(tanggal_update) = $nilaiArray[0] AND YEAR(tanggal_update) = $nilaiArray[1]";
                                    ...
                                    if (isset($array_id)) {
                                        // echo var_dump($array_id);

                                        foreach ($array_id as $array_id_value) {

                                            // echo $array_id_value; echo "<br>";
                                            $query_artikel = "SELECT * FROM artikel WHERE id_artikel = $array_id_value";
